...
struct MedicalIDCardView: View {
    ...
    @State private var walletService = WalletService.shared
                ...
                if walletService.canAddPasses {
                    Button {
                        ...
                        HStack(spacing: 8) {
                            ...
                            Text(walletPassExists ? "Actualizar en Wallet" : "Agregar a Apple Wallet")
                                .font(.nnHeadline)
                        }
                    ...
                    .disabled(walletService.isAddingPass)
    ...
    private var walletPassExists: Bool {
        guard let userId = authService.userId else { return false }
        return walletService.isPassInWallet(userId: userId.uuidString)
    }
    ...
    private func addToWallet() async {
        ...
        let contactName = emergencyContacts.first?.name ?? ""
        let contactPhone = emergencyContacts.first?.phone ?? ""

        let pass = await walletService.addToWallet(
        ...
        if let pass {
            ...
            walletError = walletService.error
        }
    }
...
// MARK: - Add Pass View (Apple Wallet)
