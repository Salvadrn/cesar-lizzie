...
struct SettingsView: View {
    ...
    @State private var fontScaleTask: Task<Void, Never>?
    @State private var showInviteQR = false
    @State private var familyVM = FamilyViewModel()
    ...
    var body: some View {
        Form {
                ...
                VStack(alignment: .leading) {
                    ...
                    Slider(value: $fontScale, in: 0.8...1.5, step: 0.1)
                        .onChange(of: fontScale) { _, newValue in
                            fontScaleTask?.cancel()
                            fontScaleTask = Task {
                                try? await Task.sleep(for: .milliseconds(500))
                                guard !Task.isCancelled else { return }
                                await vm.updateFontScale(newValue)
                            }
                        }
                }
            }
            ...
            // QR para vincular cuidador (solo pacientes)
            if authService.isPatient && !authService.isGuestMode {
                Section {
                    Button {
                        if familyVM.generatedCode == nil {
                            Task { await familyVM.generateInvite() }
                        }
                        showInviteQR = true
                    } label: {
                        HStack(spacing: 12) {
                            ...
                            VStack(alignment: .leading, spacing: 2) {
                                ...
                                Text("Genera un QR para que tu cuidador se conecte")
                                    .font(.nnCaption)
                                    .foregroundStyle(.secondary)
                            }
                            Spacer()
