...
struct AdaptAiComplicationEntry: TimelineEntry {
    let date: Date
...
struct ComplicationProvider: TimelineProvider {
    func placeholder(in context: Context) -> AdaptAiComplicationEntry {
        AdaptAiComplicationEntry(date: .now, nextRoutineTitle: "Higiene Matutina", dailyProgress: 0.6)
    }

    func getSnapshot(in context: Context, completion: @escaping (AdaptAiComplicationEntry) -> Void) {
        completion(placeholder(in: context))
    }

    func getTimeline(in context: Context, completion: @escaping (Timeline<AdaptAiComplicationEntry>) -> Void) {
        let entry = AdaptAiComplicationEntry(
            date: .now,
