...
public final class APIClient {
    ...
    // MARK: - Helpers (used by extensions)
    ...
    public func fetchTodayExecutions() async throws -> [ExecutionRow] {
        ...
        let executions: [ExecutionRow] = try await supabase
            ...
            .gte("started_at", value: startString)
            .execute()
            .value
        return executions
    }
    ...
    public func createAlert(type: AppConstants.AlertType, severity: AppConstants.AlertSeverity, title: String, message: String?) async throws {
        try await createAlert(type: type.rawValue, severity: severity.rawValue, title: title, message: message)
    }

    public func notifyFamilyMembers(alertType: AppConstants.AlertType, title: String, message: String?) async throws {
        try await notifyFamilyMembers(alertType: alertType.rawValue, title: title, message: message)
    }
